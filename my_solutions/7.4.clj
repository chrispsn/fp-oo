...
; Produces a list of class symbols (lowest-down at end);
; note that class-symbol-above grabs the SUPERCLASS symbols,
; not the CLASS symbols.
(def lineage-1
...
; Runs lineage-1, starting it with an empty list.
(def lineage
     (fn [class-symbol]
       (lineage-1 class-symbol [])))

; Gets a bunch of maps containing methods (one per class-symbol in
; the list produced by lineage) and merges them.
; Remember: merge resolves key clashes by picking from the *rightmost* map.
(def method-cache
     (fn [class]
